...
//        }
        ...
        if(isset($request['start_date']) && $request['start_date']!=""){
            ...
            $query="SELECT n.*, count(case when un.status='send' then 1 end) as send, count(case when un.status='delivered' then 1 end) as delivered from notifications n INNER JOIN user_notifications un on n.nid=un.notification_id where start_date >='". $start_date ."' GROUP BY un.notification_id";
//            $user_notification->where('created_at','>=',$request['start_date']);
        ...
        if(isset($request['end_date']) && $request['end_date']!=""){
            ...
            $query="SELECT n.*, count(case when un.status='send' then 1 end) as send, count(case when un.status='delivered' then 1 end) as delivered from notifications n INNER JOIN user_notifications un on n.nid=un.notification_id where deletion_date <='". $end_date ."' GROUP BY un.notification_id";
