...
class Admincontroller extends BaseController {
    ...
    public function dashboard(){
        if (Session::get('AdminData')) {
		 ...
		 $device_os = DB::select("select device_os as label ,count(uid) as data from users group by device_os order by device_os desc");
		 $data['cnt_device_os'] = $device_os;

		 $device_os = DB::select("SELECT concat(' ', CAST((COUNT(*) / (SELECT COUNT(*) FROM users)) * 100 AS UNSIGNED),'% ',SUBSTR(device_model,1,20))as label,CAST((COUNT(*) / (SELECT COUNT(*) FROM users)) * 100 AS UNSIGNED) as data FROM `users` group by device_model order by data desc");
			...
			if(!empty($device_os)){
				...
				$arr_new = array("12"=>(object)array("label" => " $other% other","data"=>$other));
...
//!empty($_GET['is_active']) ? $_GET['is_active'] == 2 ? $data['is_active'] = 2 : $data['is_active'] = $_GET['is_active'] : '';
           ...
           // $search_term['groups'] = DB::select("select * from `group`");
            ...
            $users = DB::table('users')->orWhere($str)->paginate(10);
				 ...
				 $mnth_user = DB::select("select * from admin where password = md5('".$_POST['old_password']."')");
					if(!empty($mnth_user)){
						 DB::table('admin')
							 ...
							 Session::put('msg', 'Password successfully upadted');
							 Session::put('type', 'success');
					}else{
