...
function search() {
    ...
    $.ajax({
        url: url,
        type: 'POST',
        data: 'lat=' + p.lat() + '&long=' + p.lng() + '&d=' + d + '&_token=' + '{{ Session::token() }}',
        success: function (data, textStatus, xhr) {
            data = jQuery.parseJSON(data);
            clearMarkers();
            if (data.marker && data.marker.length) {
                for (var i = 0, marker; marker = data.marker[i]; i++) {
                    var lat = marker.lat;
                    ...
                    // show data:
                    $('#countUser').html(data.marker.length);
                }
            } else {
                $.gritter.add({
                    title: "Status",
                                ...
                                <div class="col-md-7">
                                    <form id="frmsubmit" enctype="multipart/form-data" method="post">
                                        ...
                                        <div class="form-group">
                                                ...
                                                <div class="maps-container" id="map_style">
                                                    @include('admin.widget_maps')
                                                </div>
